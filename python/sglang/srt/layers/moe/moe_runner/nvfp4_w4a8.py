...
def fused_experts_none_to_nvfp4_w4a8(
    ...
    block_m = (
        16
        if quant_info.marlin_fp8
        else nvfp4_w4a8_moe_block_size(hidden_states, routes_per_expert)
    )
    ...
    if quant_info.marlin_fp8:
        ...
        intermediate_size = quant_info.w2_weight.shape[1] * 16
        gemm1_n = intermediate_size * 2
        hidden_fp8, hidden_scale = sglang_per_token_quant_fp8(
        ...
        gate_up = torch.empty(
            (num_tokens * top_k, gemm1_n),
            device=hidden_states.device,
            dtype=hidden_states.dtype,
        )
        gate_up = moe_wna16_marlin_gemm(
            hidden_fp8,
            gate_up,
    ...
    if intermediate_size in supported_fused_groups:
        ...
        silu_and_mul(gate_up, down_input_bf16)
        if quant_info.marlin_fp8:
            down_input, down_scale = sglang_per_token_quant_fp8(down_input_bf16)
        else:
            down_input, down_scale = down_input_bf16, None

    # The grouped GEMM dynamically quantizes down_input per route. A top_k of
    # one here means its source row is the flattened route id.
    mul_routed_weight = (
        not runner_config.apply_router_weight_on_input and not runner_config.no_combine
    )
    if quant_info.marlin_fp8:
        ...
        route_output = moe_wna16_marlin_gemm(
            down_input,
            route_output,
            ...
            moe_block_size=block_m,
            top_k=1,
            mul_topk_weights=mul_routed_weight,
            is_ep=False,
            b_q_type=scalar_types.float4_e2m1f,
